...
public class AnalysisCrashWindow : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        addr2linePath = EditorGUILayout.TextField("NDK工具路径（addr2linePath）", addr2linePath, GUILayout.Width(1200));
        unitydebugsoPath = EditorGUILayout.TextField("unity符号表（unitydebugsoPath）", unitydebugsoPath, GUILayout.MaxWidth(1200));
        il2cppdebugsoPath = EditorGUILayout.TextField("ill2cpp符合表（il2cppdebugsoPath）", il2cppdebugsoPath, GUILayout.MaxWidth(1200));
        MyCashPath = EditorGUILayout.TextField("崩溃日志文件路径", MyCashPath, GUILayout.MaxWidth(1200));
